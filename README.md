...
```sh
python3 scripts/vendor.py /path/to/YourAddon --ref v1.0.0
...
The core exports `VERSION = "1.0.0"` and `API_VERSION = 1`. Optional modules extend only that same private object. Never load the library via a separate addon or publish the object in a global registry. Different embedded versions may coexist in either addon load order.
